...
FlagStatus IWDG_GetStatus(IWDG_STATUS_FLAG IWDG_FLAG)
{
    ...
    if ((IWDG->STS & (uint32_t)IWDG_FLAG) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
/**
*\*\name    IWDG_INT_Enable.
*\*\fun     Enables or disables the specified IWDG interrupt
 *\*\param  Cmd :
 *\*\          - ENABLE
 *\*\          - DISABLE
*\*\return  none
**/
void IWDG_INT_Enable(FunctionalState Cmd)
{
    if(Cmd == ENABLE)
    {
        IWDG->CTRL |= IWDG_CRTL_ITE;
    }
    else
    {
        IWDG->CTRL &= (~IWDG_CRTL_ITE);
...
/**
*\*\name    IWDG_Feed
*\*\fun     Feed IWDG in case reset occur.
*\*\param   none
*\*\return  none
**/
void IWDG_Feed(void)
